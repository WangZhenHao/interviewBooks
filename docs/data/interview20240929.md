...
核心特点：
1: 副作用执行 -》在渲染完成后执行，不阻塞 UI 渲染

2: 清理函数 -》return 的函数会在组件卸载或下一次 effect 执行前调用

3: 依赖数组 -》 控制 effect 何时执行

```js
useEffect(() => {
  console.log('effect执行')
  return () => console.log('清理effect')
}, [value])
```
...
effect 在渲染完成后执行 → 打印 "effect执行"

- 2: value 更新：

先执行上一次 effect 的清理 → 打印 "清理effect"

再执行新的 effect → 打印 "effect执行"
...
handleClick 都是新的函数引用

子组件即使用 React.memo 包裹，也会重新 render，因为 props 函数引用变了
...
React.memo 子组件不会因为函数变化而重新 render


- useMemo 用来缓存函数执行的结果。如每次渲染时都要执行一段很复杂的运算，或者一个变量需要依赖另一个变量的运算结果，就都可以使用 useMemo()。

函数组件每次 state/props 更新，整个组件函数都会重新执行：
```js
function Demo({ a, b }) {
  const result = heavyCompute(a, b) // 每次 render 都执行
...
useMemo 可以缓存上一次的结果，只有依赖变化时才重新计算。
...
事件循环（Event Loop）是 JavaScript 在 单线程 模型下，实现异步任务调度的机制。
